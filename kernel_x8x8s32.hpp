...
gbp(dim_t k, const uint8_t *__restrict__ MP_A, const uint8_t *__restrict__ MP_B,
    ...
    for (dim_t p = 0; p < real_k_2; p += 2) {
        ...
        for (int j = 0; j < COLS / 4; j += 4) {
            ...
            auto Bk1_1
                    = cast<uint8_t>(vec_type_t<uint32_t> {MT_B[j + COLS + 1]});
            auto Bk1_2
                    = cast<uint8_t>(vec_type_t<uint32_t> {MT_B[j + COLS + 2]});
            auto Bk1_3
                    = cast<uint8_t>(vec_type_t<uint32_t> {MT_B[j + COLS + 3]});
            for (int i = 0; i < ROWS / VLEN; i++) {
                ...
                auto Ak1 = cast<uint8_t>(vec_type_t<uint32_t>::load_hinted(
                        &MT_A[i * VLEN + ROWS]));
                const vuint8 a1 = Ak1;
