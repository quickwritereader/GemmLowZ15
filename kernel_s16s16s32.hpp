...
inline
    ...
    if(fastpath){
        ...
        auto vec_alpha = VecType<float>{alpha}.vec(); 
        for (int j = 0; j < COLS; j++) {
            for (int i = 0; i < ROWS / VLEN; i++) {
                vType *C_ij = (vType *)&gPtr(i * VLEN, j);
...
inline
    ...
    if(fastpath){
            asm("");
        for (int j = 0; j < COLS; j++) {
            auto C_ij = VecType<T>::loadLen(&gPtr(0, j), BYTE_INDEX);
            C_ij += Caux[j];
        ...
        for (int j = 0; j < COLS; j++) {
            ...
            C_ij += VecType<T>{vec_signed(C_ij_float)};
            C_ij.storeLen(&gPtr(0, j), BYTE_INDEX);
        }
...
typename std::enable_if<(M >= ROWS), void>::type LoopOne_TAIL(
    dim_t m, dim_t k, float alpha,int16_t *Apacked, int16_t *Bpacked,  int32_t *C, dim_t ldC) {
    // end of the roll
}
...
inline void LoopOne(dim_t m, dim_t k, float alpha,int16_t *Apacked, int16_t *Bpacked, 
                    int32_t *C, dim_t ldC) {
    for (dim_t i = 0; i < m / ROWS; i++) {
        gbp<ROWS, COLS, int32_t, int16_t>(k, alpha, &Apacked[i * ROWS * k], Bpacked,
                                           &gPtr(i * ROWS, 0), ldC);
    }
    dim_t II = m - m % ROWS;
    if (m > II)
        LoopOne_TAIL<1, ROWS, COLS>(m - II, k,  alpha,&Apacked[II * k], Bpacked,
                                    &gPtr(II, 0), ldC);
}

template <int N, int COLS>
typename std::enable_if<(N >= COLS), void>::type LoopTwo_TAIL(
    dim_t m, dim_t n, dim_t k, float alpha,int16_t *Apacked, int16_t *Bpacked,  int32_t *C,
    dim_t ldC) {
    // end of the roll
}
...
template <int COLS>
inline void LoopTwo(dim_t m, dim_t n, dim_t k, float alpha,int16_t *Apacked, int16_t *Bpacked,  int32_t *C, dim_t ldC) {
